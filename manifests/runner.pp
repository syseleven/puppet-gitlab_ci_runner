...
define gitlab_ci_runner::runner (
  ...
  if $supports_deferred {
    ...
    $_config.keys.each |$key| {
      if $key in Array(Gitlab_ci_runner::Register_parameters) {
        fail("\$config contains a configuration key ${key} which is meant for the registration, but not for the config file. Please remove it (or upgrade to Puppet 6 ;))!")
      }
    }
